-- Bump per-call statement_timeout for the market RPCs so the aggregation
-- queries don't hit the default 8s ceiling. Each RPC scoped via SET LOCAL
-- so the timeout extension applies only to its own execution and reverts
...
begin
  -- 60s budget — single market aggregations finish in 1-3s warm, 5-15s
  -- cold; allow headroom for nationwide queries.
